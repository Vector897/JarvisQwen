...
def push_telegram(db: Session, text: str) -> tuple[bool, str]:
    ...
    if not token or not chat_id:
        return False, "未配置 Telegram Bot Token / Chat ID"
    try:
        resp = httpx.post(
        ...
        if resp.status_code == 200:
            return True, "已推送到 Telegram"
        return False, f"Telegram 推送失败：{resp.text[:200]}"
    except Exception as e:  # noqa: BLE001
        return False, f"Telegram 推送异常：{e}"
...
def push_email(db: Session, subject: str, body: str) -> tuple[bool, str]:
    ...
    if not host or not to_addr:
        return False, "未配置 SMTP 服务器 / 收件地址"
