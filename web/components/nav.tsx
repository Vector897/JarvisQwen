  ...
  return (
    <>
      ...
      <nav className="hidden md:flex w-52 shrink-0 flex-col gap-1 border-r border-slate-200 bg-white p-4">
        <Link href="/dashboard" className="mb-4 text-lg font-bold">AAOS</Link>
        {items.map(([href, icon, label]) => (
          <Link key={href} href={href}
            className={`rounded-lg px-3 py-2 text-sm transition-colors ${pathname.startsWith(href) ? "bg-slate-900 text-white" : "hover:bg-slate-100"}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-around border-t border-slate-200 bg-white py-2 md:hidden">
        ...
        <Link href="/help"
          className={`flex flex-col items-center text-xs ${pathname.startsWith("/help") ? "text-slate-900 font-bold" : "text-slate-400"}`}>
          <span className="text-lg">❓</span>帮助
        </Link>
      </nav>
    </>
  );
